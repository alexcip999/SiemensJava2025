...
public class InternshipApplicationTests {
	...
	void processItemsAsync_ShouldProcessAllItems() {
		...
		for (Long id : testItemIds) {
			Item item = new Item();
			item.setId(id);
			item.setStatus(ItemStatus.NEW.name());

			when(itemRepository.findById(id)).thenReturn(Optional.of(item));

			Item processedItem = new Item();
			processedItem.setId(id);
			processedItem.setStatus(ItemStatus.PROCESSED.name());

			lenient().when(itemRepository.save(any(Item.class))).thenReturn(processedItem);
		}
